...
std::shared_ptr<std::vector<Detection> > HumanDetector::parse_dnn_output(const std::vector<cv::Mat>& detections, cv::Mat* img, bool show_detections) {
    ...
    for (std::size_t i = 0; i < detections.size(); i++) {
        ...
        for (int j = 0; j < detections[i].rows; ++j, data+=detections[i].cols) {
            ...
            if ((confidence > detection_probability_threshold) && (classes[classIdPt.x] == "person")) {
...
std::shared_ptr<std::vector<Detection> > HumanDetector::detect(cv::Mat& prepped_img, bool show_detections) {
    cv::Mat blob;
    cv::dnn::blobFromImage(prepped_img, blob, 1/255.0, cv::Size(img_dim_[0], img_dim_[1]), cv::Scalar(0,0,0), true, false);        
    net.setInput(blob);
